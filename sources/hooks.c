...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/28 19:25:18 by ahiroko           #+#    #+#             */
/*   Updated: 2019/07/28 19:25:21 by ahiroko          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			key_controls(int key, t_fdf *fdf)
{
	...
	if ((key == KB_KEY_RIGHT || key == KB_KEY_LEFT ||
		key == KB_KEY_UP || key == KB_KEY_DOWN) && fdf->cam->menu == CLOSED)
		move(key, fdf);
	...
	if (key == KB_KEY_C && fdf->cam->menu == CLOSED)
		change_color(fdf);
	if ((key == KB_NUM_4 || key == KB_NUM_6 || key == KB_NUM_8 ||
		key == KB_NUM_5 || key == KB_NUM_7 || key == KB_NUM_9) &&
		fdf->cam->menu == CLOSED)
		rotate(key, fdf);
